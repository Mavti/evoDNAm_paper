...
# I need contingency table of clusters and evo divergence
...
for (sp in sp_vector) {
  for ( tf in tf_vector) {
    ...
    title<-paste0("Species cons vs Clusters - ",tf," - ",sp)
    gplots::balloonplot(t(tab), main =title, xlab ="", ylab="",
    ...
    title<-paste0(tf," - ",sp)
    p<- paste0("/nfs/research/flicek/user/rimoldi/Projects/EDM/Figures/CA/", title,".pdf")
    association <- corrplot(chisq$residuals, is.cor = FALSE,title = title,  col = brewer.pal(10,'PRGn'))
    pdf(p)
    association
    dev.off()
    association
